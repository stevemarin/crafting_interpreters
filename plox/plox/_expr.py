...
class Binary(Expr):
        ...
        match self.operator.type:
            ...
            case TokenType.PLUS:
                if isinstance(left, float) and isinstance(right, float):
                    ...
                    raise RuntimeError(
                        f"{self.operator} operarands must both "
    ...
    def check_number_operands(operator: Token, left: Any, right: Any) -> None:
        if isinstance(left, float) and isinstance(right, float):
            ...
            raise RuntimeError(
                f"{operator}: operands {left} " + f"and {right} must be numbers"
            )
...
class Variable(Expr):
    ...
    def resolve(self, resolver: "Resolver") -> None:
        if (
            len(resolver.scopes) > 0
            and self.name.lexeme in resolver.scopes[-1]
            and not resolver.scopes[-1][self.name.lexeme]
        ):
            from plox.lox import Lox

            Lox.error_token(
                self.name, "Can't read local variable in it's own initializer."
            )
...
class Super(Expr):
    ...
    def resolve(self, resolver: "Resolver") -> None:
        ...
        if resolver.current_class == ClassType.NONE:
            ...
            Lox.error_token(
                self.keyword, "Can't use 'super' in a class with not superclass."
            )
